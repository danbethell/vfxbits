...
# OSX: export DYLD_LIBRARY_PATH=$VFXLIBS/lib
# OSX: export DYLD_FRAMEWORK_PATH=$VFXLIBS/lib


# Linux Requirements:
# - Reasonably modern distro. GCC/G++ compiler. Bash & Csh. Zlib. Bzip2dev. Inkscape.
# Linux: export LD_LIBRARY_PATH=$VFXLIBS/lib
# Linux: install zlib (sudo apt-get install zlib1g-dev)
# Linux: install csh (sudo apt-get install csh)
# Linux: install inkscape (sudo apt-get install inkscape)
# Linux: install libbz2-dev
# Linux: install doxygen
# Linux: install llvm-dev, flex, bison
...
# OSX: require patch to remove -lSystemStubs from L7230 of Python-2.6.8/configure
cd ${STAGING}/Python-2.6.8
# OSX: ./configure --prefix=${VFXLIBS} --enable-framework=${VFXLIBS}/lib --enable-unicode=ucs4
# Linux: ./configure --prefix=${VFXLIBS} --enable-shared --enable-unicode=ucs4
./configure --prefix=${VFXLIBS} --enable-shared --enable-unicode=ucs4
make clean; make -j 8; make install
# # OSX: cd ${VFXLIBS}/bin; ln -fsh ../lib/Python.framework/Versions/Current/bin/python python
...
# make clean; make -j 8;
# # OSX: cp build/macos_*_release/*.dylib $VFXLIBS/lib; cp -r include/tbb $VFXLIBS/include
# # Linux: cp build/linux_*_release/*.so $VFXLIBS/lib; cp -r include/tbb $VFXLIBS/include
...
# OSX: cp -r dist/macosx/* $VFXLIBS
# OSX: cp $VFXLIBS/lib/libOpenImageIO.dylib $VFXLIBS/lib/libOpenImageIO-1.dylib
# Linux: cp -r dist/linux64/* $VFXLIBS
# Linux: cp $VFXLIBS/lib/libOpenImageIO.so $VFXLIBS/lib/libOpenImageIO-1.so
...
# you'll get prompted to accept the FOSS license by configure
...
mkdir -p build
cd build
# OSX: $VFXLIBS/bin/cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS='-w' -DCMAKE_INSTALL_PREFIX=$VFXLIBS -DPYTHON_EXECUTABLE=$VFXLIBS/lib/Python.framework/Versions/2.6/bin/python -DPYTHON_LIBRARY=$VFXLIBS/lib/Python.framework/Versions/2.6/lib/python2.6/config/libpython2.6.dylib -DPYTHON_INCLUDE_DIR=$VFXLIBS/lib/Python.framework/Versions/2.6/include/python2.6
# Linux: $VFXLIBS/bin/cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS='-w' -DCMAKE_INSTALL_PREFIX=$VFXLIBS -DPYTHON_EXECUTABLE=$VFXLIBS/bin/python -DPYTHON_LIBRARY=$VFXLIBS/lib/libpython2.6.so -DPYTHON_INCLUDE_DIR=$VFXLIBS/include/python2.6
